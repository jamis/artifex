...
module.exports =
  ...
  "should have two L1 atWill powers": (test) ->
    ...
    for power in npc.powers.atWill
      test.ok power.id in Classes.Fighter.powers.atWill[1]
    test.done()
  ...
  "should have one L1 encounter power": (test) ->
    ...
    for power in npc.powers.encounter
      test.ok true if power.id in Classes.Fighter.powers.encounter[1]
    test.done()
  ...
  "should have one L1 daily power": (test) ->
    ...
    for power in npc.powers.daily
      test.ok true if power.id in Classes.Fighter.powers.daily[1]
    test.done()
